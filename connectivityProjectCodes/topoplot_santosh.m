% changes at "1148" line by Santosh W
...
function [handle,Zi,grid,Xi,Yi] = topoplot_santosh(Values,loc_file,varargin)
...
if ~strcmpi(STYLE,'blank') % if draw interpolated scalp map
 if ~strcmpi(STYLE,'grid') %  not a rectangular channel grid
  ...
  elseif strcmp(STYLE,'contour')                     % plot surface contours only
    ...
    handle.LevelList = handle.LevelList(3); % added by Santosh
    for h=chs, set(h,'color',CCOLOR,'LineWidth',2); end % changed by Santosh
  %
